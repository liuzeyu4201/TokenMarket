...
```bash
bash .claude/skills/merge-reconciler/scripts/_common.sh [incoming-ref]
```
...
1. 跑分类器：
   ```bash
   bash .claude/skills/merge-reconciler/scripts/classify-blobs.sh [incoming-ref]
   ```
...
2. **标记行为扫描**：找某侧专属行为被丢（如 desktop 隐藏标签条件）：
   ```bash
   bash .claude/skills/merge-reconciler/scripts/scan-markers.sh [marker-regex] [OURS|THEIRS]
   ```
3. **前向点状扫描**：找某侧前向新增的具体行被丢：
   ```bash
   bash .claude/skills/merge-reconciler/scripts/scan-lost-forward.sh [OURS|THEIRS]
   ```
...
- 默认过滤噪音目录（`client/ specs/ docs/ .specify/ .agents/ .codex/ .claude/`）；可用 env `NOISE_RE` 覆盖。
