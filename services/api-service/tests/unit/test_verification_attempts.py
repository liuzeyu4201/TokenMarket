...
states expired/locked/consumed/superseded, and decoy uniform actions.
"""
...
def _challenge(
    *,
    ...
    digest = otp_verification_digest(otp_mat.current, cid, salt, plain)
    ...
    ch.expires_at = now + expires_delta
...
) -> tuple[SessionService, AsyncMock]:
    session = AsyncMock()
    settings = _settings()
...
async def test_malformed_code_is_validation_without_attempt(bad_code: str) -> None:
    ...
    service, _ = _service_with_challenge(ch)
...
async def test_wrong_code_attempts_one_through_four_retry() -> None:
    ...
    user.role = "buyer"
    user.nickname = "u"
    user.phone_normalized = "13800138000"

    ch = _challenge(user_id=user_id, attempt_count=0)
    service, _ = _service_with_challenge(ch, user=user)
...
async def test_decoy_correct_code_uses_attempt_lifecycle() -> None:
    ...
    service, _ = _service_with_challenge(ch, user=None)
    result = await service.create_session(
        challenge_id=ch.id,
        code=ch._plain,
        request_id="r-decoy",
