...
func TestContractAssetsPresentAndOpenAPIParseable(t *testing.T) {
	root := repoRoot(t)
	dir := filepath.Join(root, "shared", "contracts", "volcano-key-validation", "v1")
	files := []string{
	...
	raw, err := os.ReadFile(filepath.Join(dir, "volcano-key-validation.openapi.yaml"))
	if err != nil {
		t.Fatal(err)
	}
...
func repoRoot(t *testing.T) string {
	...
	// .../services/proxy-gateway/internal/domain/providervalid/contract_assets_test.go
