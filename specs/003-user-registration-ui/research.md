...
## Decision 1: 账户域归属 API Service

**Decision**: 用户注册用例、`users` 表与注册幂等表由 **API Service** 单独拥有。Billing/Admin/Gateway 不读写用户表。前端在本地开发中直接调用 API Service 的版本化业务路径；Gateway 仍只负责 AI 代理流量，本功能不扩展 Gateway 业务注册路由。

**Rationale**: 宪章要求域规则与持久化同属一个服务；SF01 已将 API Service 定为应用域服务并具备 PostgreSQL readiness。注册不涉及代理计量。
...
## Decision 2: 统一业务响应包络 + OpenAPI 契约先行

**Decision**: 所有注册相关 HTTP 响应使用版本化包络：
...
**Rationale**: 产品全局规范与 SF03 要求统一包络；机读契约满足宪章 contract-first 与前端类型对齐。
...
## Decision 3: 手机号规范化规则（大陆 11 位 + 噪声清洗）

**Decision**: 单一纯函数 `normalize_cn_mobile(raw) -> Result[str, ValidationError]`：
...
**Rationale**: 澄清会话锁定范围；可单测矩阵覆盖噪声变体与非大陆号。
...
## Decision 4: 角色枚举包含 `both`，不含密码/邮箱

**Decision**: V0.1 用户角色为 PostgreSQL ENUM `buyer | seller | both`。不建 `password_hash`、不强制 `email`。状态 ENUM 至少含 `active`（新建默认）；可预留 `suspended` 但不在本功能流转。`admin` 角色不在本功能创建路径出现。

**Rationale**: 对齐 SF03 与澄清后的规格；周度 Spec 中的明文密码被宪章禁止，且本功能明确不签发会话。
...
## Decision 5: 幂等事实存 PostgreSQL，24h 窗口

**Decision**: 表 `registration_idempotency_records` 为幂等权威源（非 Redis 唯一存储）：
...
**Rationale**: 宪章规定 Redis 不得作为耐久事实唯一副本；注册结果必须可在 DB 恢复后重放。24h 来自澄清。
...
## Decision 6: 限流用 Redis 固定窗口，失败关闭可降级策略
...
**Rationale**: 宪章允许 Redis 作 rate-limit 存储；双维度满足 FR-018/019。Fail-closed 优先防滥用。
...
## Decision 7: 软删除手机号专用业务码

**Decision**: `phone_normalized` 在 **全表**（含 `is_deleted = true`）唯一。查到已删账户 → `code=ACCOUNT_UNAVAILABLE`，**HTTP 409**（与 `contracts/business-codes.md` 一致，不用 422），文案中性“账户不可用，请通过恢复流程处理”，不泄露昵称/角色。活跃占用 → `PHONE_ALREADY_REGISTERED`。两者前端可区分。

**Rationale**: 澄清选项 B；全表唯一防止静默重建。
...
**Rationale**: 规格要求最小可导航壳层 + 注册页；与前端规范目录演进兼容且依赖增量最小。
...
**Decision**: Alembic 修订 `0002_users_registration`（名称以实施为准）在 `api-service`：
...
**Rationale**: 宪章迁移纪律；API Service 已有 baseline 与迁移入口。

## Decision 10: 可观测性指标、告警与脱敏
...
**Rationale**: FR-011、SC-005、ER-006、ER-006a；宪章 VI 新失败模式需检测与 runbook。

## Open items deferred to implementation tasks (not product unknowns)
