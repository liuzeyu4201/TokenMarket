...
> **当前激活状态**
>
> SF02 公共生命周期已在 T074 激活。默认 `make start` / `make stop` 会启动或
> 停止中间件与五个主机应用进程；`make dev` / `make dev-down` 仅管理中间件。
...
| 错误码                   | 含义                                       | 恢复                                             |
| ------------------------ | ------------------------------------------ | ------------------------------------------------ |
| `SF02_NOT_READY`       | 历史诊断码；公共入口激活后不应再出现       | 升级到已激活的 T074 工具链；检查是否调用了过期脚本 |
